...
- `client_message_worker.py`: Processes client messages during workflow execution. Supports checkbox-based control (update_description, add_tasks, provide_answer) to explicitly direct the LLM's behavior, or direct passthrough mode when no checkboxes are selected (the user message is sent to the LLM as-is). Uses specialized prompts based on checkbox combinations (see CHECKBOX_PROMPTS.md). Changes to description and tasks are detected in the workflow_runner to display appropriate status messages in the chat panel.
