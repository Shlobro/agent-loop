...
- `review_worker.py`: Runs an optional pre-review unit-test-update pass (uses `git diff` and may add/edit tests) before review/fix cycles per selected review type (including UI/UX), initializes `review/` with empty files for every review type, reads findings from the current review file, skips fixer when that file is empty, truncates the same file after each completed fix cycle, and supports live updates of review iteration limits and reviewer/fixer model selections between cycles.
...
- `MainWindow` can pass live runtime settings into `ReviewWorker` so debug/review iterations and reviewer/fixer choices can be changed while the loop is running.
