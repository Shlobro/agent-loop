...
- `file_manager.py`: Atomic read/write for `tasks.md`, `recent-changes.md`, `review.md`, `product-description.md`, governance prompt files (`AGENTS.md`, `CLAUDE.md`, `GEMINI.md`), and workspace rule compliance scans (one developer guide `.md` per folder with a root exception, <=10 code files per folder with `.md` excluded, <=1000 lines per code file). Includes cache invalidation on working directory changes.
