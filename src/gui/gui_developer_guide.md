...
- Expose menu actions for `Workflow` (run controls) and `View` (status and left panel tab toggles) to preserve a minimalist default layout. The View menu provides 4 toggles: Show Status Panel (top bar), Show Logs (left tab), Show Description (left tab), and Show Tasks (left tab). All tabs default to hidden. The left panel only appears when at least one tab is enabled. The right column always contains only the chat panel, never showing description or tasks above it.
