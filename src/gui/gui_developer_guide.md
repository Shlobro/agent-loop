...
- `MainWindow` drives chat UX lifecycle hooks: it always appends user chat entries (including first-message description initialization), sets an animated bot activity line (`Generating questions...`) when question generation starts, clears it when questions are ready, and posts a one-line bot completion message.
