...
- `question_flow_decision_dialog.py`: Non-modal post-rewrite decision dialog that appears after answers are folded into `product-description.md`. It keeps question flow paused until the user explicitly chooses one of four actions: `Ask More Questions`, `Edit Product Description` (opens a dedicated modal editor window), `Continue` (closes the decision dialog and returns to the main window), or `Start Main Loop`. The explanatory message is styled with larger text for clearer decision prompts.
...
- Update post-rewrite branching behavior (`Ask More Questions`, `Edit Product Description`, `Continue`, `Start Main Loop`): `question_flow_decision_dialog.py`.
