...
- Add new LLM provider/model or CLI flags (including output-file capture and prompt transport mode): `llm/*_provider.py` plus `gui/widgets/llm_selector_panel.py`. Provider files hold the curated model ID lists shown in the UI; Claude, Gemini, and Codex send prompts via stdin.
