...
- Review loop: `workers/review_worker.py`, `llm/prompt_templates.py`, `core/file_manager.py`, `gui/main_window.py` (includes General, Unit Test, and UI/UX review types, pre-creates per-type review files under `review/` when a valid working directory becomes active, and skips fixer when the active review file is empty).
