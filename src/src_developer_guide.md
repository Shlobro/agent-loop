...
- Main execution: `workers/execution_worker.py`, `llm/prompt_templates.py`, `core/file_manager.py`, `utils/markdown_parser.py`, `gui/widgets/task_loop_panel.py`, `gui/widgets/status_panel.py`, `gui/main_window.py` (loop UI highlights current action plus Markdown-rendered completed/incomplete task lists and task-count-based progress).
