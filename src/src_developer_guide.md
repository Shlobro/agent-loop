...
- Main execution: `workers/execution_worker.py`, `llm/prompt_templates.py`, `core/file_manager.py`, `utils/markdown_parser.py` (includes workspace compliance checks: read `product-description.md` first, each folder must have a `developer-guide.md` [max 500 lines] updated on change, read guides before editing, update ancestor guides, no legacy info, <=10 code files per folder, <=1000 lines per code file, fresh scan each run).
