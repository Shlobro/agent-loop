...
1. On launch, the app first requires selecting a working directory (with recent-directory shortcuts). After selection, the default shell is intentionally minimalist: a `Product Description` header and the description input are the only visible workspace elements. Workflow commands are available from the `Workflow` menu, and optional surfaces (status, logs, control buttons, description view controls, chat panel) are opt-in via the `View` menu. A floating circular start button appears only when the app is idle and the description is non-empty; it uses a forward icon and triggers the same start path as the workflow start action. Execution controls are edited from `Settings -> Configuration Settings`, LLM provider/model choices from `Settings -> LLM Settings`, and review type selection from `Settings -> Review Settings`. The description is synced to `product-description.md`, and the GUI value is force-written to that file before each question batch and before planning. After answers are submitted, only the current Q&A batch is rewritten into `product-description.md` using `PromptTemplates.format_definition_rewrite_prompt`; only this rewrite step pushes file content back into the GUI. Stored Q&A context is then cleared so the rewritten description is treated as the new initial input. The description panel provides three view modes: `Edit` (default), `Preview` (Markdown rendering), and `Task List` (shows current action, completed/incomplete task counts and lists). After each rewrite, a non-modal decision dialog keeps the flow paused until the user explicitly selects either `Ask More Questions` or `Start Main Loop`, so description edits can continue before planning starts. During a run, configuration and LLM selections remain editable and are applied to upcoming phases and iterations. Once the main loop starts, the description panel automatically switches to Task List mode and shows the view controls, displaying live agent action plus completed/incomplete Markdown-rendered task lists in the full content area, and the top-right percent is driven by task completion ratio from `tasks.md` (not by iteration limit). Users can send client messages via the chat panel (View → Show Chat Panel) at any time when a working directory is active. During workflow execution, messages queue and are processed at iteration boundaries after git operations. When the workflow is idle, messages are processed immediately. The LLM analyzes each message and either updates product-description.md/tasks.md or provides an answer in answer.md shown via modal dialog.
