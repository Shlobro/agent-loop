...
- LLM output capture and prompt transport behavior (argv vs stdin/output-file), including subprocess cwd validation/fallback when configured paths are invalid: `src/workers/llm_worker.py`, `src/llm/*_provider.py` (Claude, Gemini, and Codex use stdin prompts). Current defaults: 600-second base timeout, 1200-second execution-stage timeout override, and 4-second retry delay for retrying worker calls.
